...
export function generateItinerary(
  ...
  const activitiesPerDay = Math.floor(
    selectedActivities.length / Math.max(duration - 2, 1),
  );
  let activityIndex = 0;
...
export function calculateBudget(
  ...
  const mealsCost = itinerary.reduce((sum, day) => {
    const dayCost = day.activities
      .filter(
        (a) =>
          a.activity.toLowerCase().includes("lunch") ||
          a.activity.toLowerCase().includes("dinner") ||
          a.activity.toLowerCase().includes("breakfast"),
      )
...
export function generateTripPlan(
  ...
  const recommendations = {
    restaurants: destinationInfo?.highlights
      .slice(0, 3)
      .map((h) => (typeof h === "string" ? h : h.title || h.toString())) || [
    ...
    tips: [
      ...
      `Keep emergency numbers: ${property.contact}`,
